...
{
  return FilePath::Combine(GetUserDocumentsDirectory(), "ZeroProjects");
}
...
bool NewProjectArea::Accept(StringParam projectName, StringParam location)
{
  ...
  String file = FilePath::CombineWithExtension(directory, projectName, ".zeroproj");
  ...
  CreateProject(Z::gEditor, projectName, directory, editorMode);

  // Restart the editor
  if(!Os::IsDebuggerAttached())
  {
    // Store the new project in the config file
    EditorConfig* config = HasOrAdd<EditorConfig>(Z::gEditor->mConfig);
  ...
  : Composite(parent)
{
  ...
  mNameBox->TakeFocus();
  Z::gEditor->SetExploded(true, true);
}
